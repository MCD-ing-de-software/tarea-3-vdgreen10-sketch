...
def make_sample_df() -> pd.DataFrame:
    ...
    return pd.DataFrame(
        {
        "name": ["Alice", None, "Charlie", "Diana", "Eve", "Frank", "Grace", "Erick"],
        "age":  [20, 22, 25, None, 30, 35, 40, 120],
        "city": ["NY", "LA", "SF", "TX", "NY", "LA", "SF", "TX"],
        }
    )
...
class TestDataCleaner(unittest.TestCase):
    ...
    def test_trim_strings_strips_whitespace_without_changing_other_columns(self):
        ...
        # Crear DataFrame con espacios en blanco
        df = make_sample_df()
        cleaner = DataCleaner()
        df_original = df.copy()
        ...
        self.assertEqual(df_original.loc[0, "name"], df.loc[0, "name"])

        self.assertEqual(result.loc[0, "name"], df.loc[0, "name"].strip())
    ...
    def test_remove_outliers_iqr_removes_extreme_values(self):
        ...
        df = make_sample_df()  # contiene un valor extremo en "age" (ej: 120)
        cleaner = DataCleaner()

        result = cleaner.remove_outliers_iqr(df, col="age", factor=1.5)

        # Verifica que el valor extremo (120) fue eliminado
        self.assertNotIn(120, result["age"].values)

        # Verifica que al menos uno de los valores normales permanece
    ...
    def test_remove_outliers_iqr_raises_keyerror_for_missing_column(self):
        ...
        with self.assertRaises(KeyError):
            cleaner.remove_outliers_iqr(df, col="salary", factor=1.5)
    ...
    def test_remove_outliers_iqr_raises_typeerror_for_non_numeric_column(self):
        ...
        with self.assertRaises(TypeError):
            cleaner.remove_outliers_iqr(df, col="city", factor=1.5)
